...
def process_log():
	...
	for error in errors:
		if is_error_new(error):
			...
			result = fetch(error)
			print(f"KB article for the error is ", result)

# Get KB article from DB
def fetch(error_message): 
	# For creating Database
	conn = sqlite3.connect('test.db')
	cursor = conn.cursor()
	cursor.execute('SELECT kb_article from errors where error_message = ?', (error_message,))
